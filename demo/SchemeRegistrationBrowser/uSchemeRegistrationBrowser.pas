...
uses
  ...
  uCEFTypes, uCEFConstants, Vcl.Buttons;
...
procedure TSchemeRegistrationBrowserFrm.btn1Click(Sender: TObject);
begin
  Chromium1.LoadURL('file:///' + ExtractFilePath(Application.ExeName) + 'scheme.html');
end;
...
procedure TSchemeRegistrationBrowserFrm.Chromium1GetResourceHandler(
  Sender: TObject; const browser: ICefBrowser; const frame: ICefFrame;
  const request: ICefRequest; out Result: ICefResourceHandler);
var
...
procedure TSchemeRegistrationBrowserFrm.Chromium1Jsdialog(Sender: TObject;
...
procedure TSchemeRegistrationBrowserFrm.Chromium1LoadStart(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame; transitionType: Cardinal);
begin
  ShowMessage(frame.Url);
end;
